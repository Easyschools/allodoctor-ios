...
class SearchViewController: BaseViewController<SearchViewModel> {
    ...
    override func setupUI() {
        ...
        // Setup area dropdown for both modes
        citiesDropDownVIew.setDropdownHeight(500)
        citiesDropDownVIew.label.text = AppLocalizedKeys.selectArea.localized
        citiesDropDownVIew.isHidden = false

        switch viewModel.displayMode {
        case .globalSearch:
            searchResultLabel.text = AppLocalizedKeys.AllSpecialities.localized
        case .hospitalSpecialties:
            searchResultLabel.text = AppLocalizedKeys.AllSpecialities.localized
        }
    }
...
extension SearchViewController: UITableViewDelegate, UITableViewDataSource {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        case .hospitalSpecialties:
            return viewModel.filteredHospitalSpecialties.count
        }
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
            ...
            if viewModel.searchText.isEmpty {
                ...
                if let searchResult = viewModel.searchResult.indices.contains(indexPath.row) ? viewModel.searchResult[indexPath.row] : nil {
                    if language == .ar {
                        ...
                        cell.cellLabel.text = searchResult.nameEn
                    }
        ...
        case .hospitalSpecialties:
            let specialty = viewModel.filteredHospitalSpecialties[indexPath.row]
            if language == .ar {
                cell.cellLabel.text = specialty.nameAr ?? specialty.name
            } else {
                cell.cellLabel.text = specialty.nameEn ?? specialty.name
            }
            cell.searchedItemType.text = AppLocalizedKeys.Speciality.localized
        }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        case .hospitalSpecialties:
            let specialty = viewModel.filteredHospitalSpecialties[indexPath.row]
            viewModel.navToHospitalDoctorSearch(specialtyId: specialty.id)
        }
    }
...
extension SearchViewController {
    ...
    func bindHospitalSpecialties() {
        viewModel.$filteredHospitalSpecialties
            .receive(on: DispatchQueue.main)
...
extension SearchViewController:SectionSearchableTableViewControllerDelegate {
    func sectionSearchableTableViewController(_ controller: SectionSearchableTableViewController, didSelectDistrictWithID id: Int, districtName name: String) {
        ...
        // Only fetch search results in global search mode
        switch viewModel.displayMode {
        case .globalSearch:
            viewModel.fetchsearchResults(searchedText:viewModel.searchText)
        case .hospitalSpecialties:
            // Store the area selection for later use (e.g., when navigating to doctors)
            break
        }
