        ...
        if index == 1{
            ...
            if booking.typeOfBooking == "booking" {
                ...
                if UserDefaultsManager.sharedInstance.getLanguage() == .ar{
                    ...
                    cell.setupCell(name: "intensive Care", date: booking.createdAt?.fullDateFormatter() ?? "No date", appointmentNumber: booking.id.toString().prepend("#"), image: .intesiveCare, status: booking.status ?? "pending")
                }
            }
