//
//  FWIPNSheetViewController.swift
...
public class FWIPNSheetViewController: UIViewController {
    public private(set) var options: FWIPNSheetOptions
    ...
    public var autoAdjustToKeyboard = FWIPNSheetViewController.autoAdjustToKeyboard
    ...
    public var allowPullingPastMaxHeight = FWIPNSheetViewController.allowPullingPastMaxHeight
	...
	public var allowPullingPastMinHeight = FWIPNSheetViewController.allowPullingPastMinHeight
    
    /// The sizes that the sheet will attempt to pin to. Defaults to intrinsic only.
    public var sizes: [FWIPNSheetSize] = [.intrinsic] {
        didSet {
            self.updateOrderedSizes()
        }
    }
    public var orderedSizes: [FWIPNSheetSize] = []
    public private(set) var currentSize: FWIPNSheetSize = .intrinsic
    ...
    public var hasBlurBackground = FWIPNSheetViewController.hasBlurBackground {
        didSet {
    ...
    public var overlayColor = FWIPNSheetViewController.overlayColor {
        didSet {
    ...
    public var blurEffect = FWIPNSheetViewController.blurEffect {
        didSet {
            self.blurView.effect = blurEffect
        }
    }
    public static var allowGestureThroughOverlay: Bool = false
    public var allowGestureThroughOverlay: Bool = FWIPNSheetViewController.allowGestureThroughOverlay {
        didSet {
    ...
    let transition: FWIPNSheetTransition
    
    public var shouldDismiss: ((FWIPNSheetViewController) -> Bool)?
    public var didDismiss: ((FWIPNSheetViewController) -> Void)?
    public var sizeChanged: ((FWIPNSheetViewController, FWIPNSheetSize, CGFloat) -> Void)?
    public var panGestureShouldBegin: ((UIPanGestureRecognizer) -> Bool?)?
    
    public private(set) var contentViewController: FWIPNSheetContentViewController
    ...
    private var panGestureRecognizer: FWIPNInitialTouchPanGestureRecognizer!
    ...
    public init(controller: UIViewController, sizes: [FWIPNSheetSize] = [.intrinsic], options: FWIPNSheetOptions? = nil) {
        let options = options ?? FWIPNSheetOptions.default
        self.contentViewController = FWIPNSheetContentViewController(childViewController: controller, options: options)
        ...
        self.transition = FWIPNSheetTransition(options: options)
        self.minimumSpaceAbovePullBar = FWIPNSheetViewController.minimumSpaceAbovePullBar
        super.init(nibName: nil, bundle: nil)
        self.gripColor = FWIPNSheetViewController.gripColor
        self.gripSize = FWIPNSheetViewController.gripSize
        self.pullBarBackgroundColor = FWIPNSheetViewController.pullBarBackgroundColor
        self.cornerRadius = FWIPNSheetViewController.cornerRadius
        self.updateOrderedSizes()
    ...
    public override func loadView() {
        if self.options.useInlineMode {
            let FWIPNSheetView = FWIPNSheetView()
            FWIPNSheetView.delegate = self
            self.view = FWIPNSheetView
        } else {
            super.loadView()
        }
    }
    ...
    public override func viewDidLoad() {
        ...
        self.FWIPNcompatibleAdditionalSafeAreaInsets = UIEdgeInsets(top: -self.options.pullBarHeight, left: 0, bottom: 0, right: 0)
    ...
    public func setSizes(_ sizes: [FWIPNSheetSize], animated: Bool = true) {
    ...
    func updateOrderedSizes() {
        var concreteSizes: [(FWIPNSheetSize, CGFloat)] = self.sizes.map {
...
//            pullBarLabel = /*FWIPNLocalize.dismissPresentation.FWIPNLocalized*/
//        } else if self.orderedSizes.count > 1 {
////            pullBarLabel = FWIPNLocalize.changeSizeOfPresentation.FWIPNLocalized
//        }
    ...
    private func addOverlay() {
        ...
        FWIPNConstraints(for: self.overlayView) {
    ...
    private func addBlurBackground() {
        ...
        FWIPNConstraints(for: self.blurView) {
            $0.edges(.top, .left, .right, .bottom).pinToSuperview()
        }
...
//        self.overlayTapView.accessibilityLabel = FWIPNLocalize.dismissPresentation.FWIPNLocalized
        FWIPNConstraints(for: overlayTapView, self.contentViewController.view) {
    ...
    private func addContentView() {
        ...
        FWIPNConstraints(for: self.contentViewController.view) {
            ...
            if (self.options.useFullScreenMode) {
                ...
                top = max(12, UIApplication.shared.windows.first(where:  { $0.isKeyWindow })?.FWIPNcompatibleSafeAreaInsets.top ?? 12)
            }
    ...
    private func addPanGestureRecognizer() {
        let panGestureRecognizer = FWIPNInitialTouchPanGestureRecognizer(target: self, action: #selector(panned(_:)))
    ...
    private func height(for size: FWIPNSheetSize?) -> CGFloat {
        ...
        if self.options.useFullScreenMode {
            ...
            fullscreenHeight = self.view.bounds.height - self.view.FWIPNcompatibleSafeAreaInsets.top - self.minimumSpaceAbovePullBar
        }
        switch (size) {
    ...
    public func resize(to size: FWIPNSheetSize,
    ...
    public func animateIn(to view: UIView, in parent: UIViewController, size: FWIPNSheetSize? = nil, duration: TimeInterval = 0.3, completion: (() -> Void)? = nil) {
    ...
    public func animateIn(size: FWIPNSheetSize? = nil, duration: TimeInterval = 0.3, completion: (() -> Void)? = nil) {
...
extension FWIPNSheetViewController: FWIPNSheetViewDelegate {
...
extension FWIPNSheetViewController: UIGestureRecognizerDelegate {
    ...
    public func gestureRecognizerShouldBegin(_ gestureRecognizer: UIGestureRecognizer) -> Bool {
        guard let panGestureRecognizer = gestureRecognizer as? FWIPNInitialTouchPanGestureRecognizer, let childScrollView = self.childScrollView, let point = panGestureRecognizer.initialTouchLocation else { return true }
        ...
        if velocity.y < 0 {
            ...
            return height(for: self.orderedSizes.last) > containerHeight && containerHeight < height(for: FWIPNSheetSize.fullscreen)
        } else {
            return true
        }
    }
...
extension FWIPNSheetViewController: FWIPNSheetContentViewDelegate {
    func pullBarTapped() {
...
extension FWIPNSheetViewController: UIViewControllerTransitioningDelegate {
